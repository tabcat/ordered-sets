 ...
 */
export function * dualTraversal <T>(first: T[], second: T[], comparator: (a: T, b: T) => number): Generator<[number, number, number]> {
  ...
  while ((i >= 0 && i < first.length) && (j >= 0 && j < second.length)) {
    ...
    const order = comparator(safeArrayAccess(first, safeI), safeArrayAccess(second, safeJ))
  ...
  while (i >= 0 && i < first.length) {
    ...
    if (j === -1) {
      ...
      yield [safeI, safeJ, comparator(safeArrayAccess(first, safeI), safeArrayAccess(second, safeJ))]
    }
